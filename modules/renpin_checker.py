...
from avilla.core.elements import Notice, Picture, Text
from avilla.core.resource import RawResource
from avilla.standard.core.application.event import ApplicationPreparing
...
async def main(ctx: Context):
    ...
    is_new, renpin, qianwen = await read_data(target)
    img_bytes = await md2img(f'{qianwen}\n\n---\n悄悄告诉你噢，你今天的人品值是：{renpin}')
    if is_new:
        await ctx.scene.send_message(
            MessageChain([Notice(ctx.client), Text(' 你抽到一支签：'), Picture(RawResource(img_bytes))])
        )
    else:
        await ctx.scene.send_message(
            MessageChain(
                [
                    Notice(ctx.client),
                    Text(' 你今天已经抽到过一支签了，你没有好好保管吗？这样吧，再告诉你一次好了，你抽到的签是：'),
