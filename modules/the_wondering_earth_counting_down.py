...
from avilla.standard.core.message.event import MessageReceived
...
from libs.path import static_path
...
IN 16 SECONDS'''
    ...
    cn_font = ImageFont.truetype(str(static_path / 'fonts' / 'HarmonyOS_Sans_SC_Bold.ttf'), 100)
    number_font = ImageFont.truetype(str(static_path / 'fonts' / 'HarmonyOS_Sans_SC_Bold.ttf'), 285)
    en_font = ImageFont.truetype(str(static_path / 'fonts' / 'HarmonyOS_Sans_SC_Bold.ttf'), 45)
    ...
    if gif.result:
        ...
        for _ in range(int(number), 0, -1):
            ...
            frames.append(
                await asyncio.to_thread(
                    the_wondering_earth_counting_down,
                    cn_1,
                    prefix,
                    num,
                    suffix,
        ...
        with BytesIO() as f:
            ...
            await ctx.scene.send_message(MessageChain([Picture(RawResource(f.getvalue()))]))
    else:
        with BytesIO() as f:
